...
class headIdDoc:
    ...
    def extend(self, wordindices, dataset_set_characters, dataset_set_bio_tags, dataset_set_ec_tags,
        ...
        for tId in range(len(self.tokens)):
            self.embedding_ids.append(int(data_utils.getEmbeddingId(self.tokens[tId], wordindices)))
            self.char_ids.append(data_utils.tokenToCharIds(self.tokens[tId], dataset_set_characters))
            self.BIO_ids.append(int(data_utils.getLabelId(self.BIOs[tId], dataset_set_bio_tags)))
            self.ecs.append(data_utils.getECfromBIO(self.BIOs[tId]))
            self.ec_ids.append(int(data_utils.getLabelId(data_utils.getECfromBIO(self.BIOs[tId]), dataset_set_ec_tags)))
            self.joint_ids.append(data_utils.getScoringMatrixHeads(self.relations[tId], dataset_set_relations, self.heads[tId]))
        ...
        for i in range(file.shape[0]):
            if '#doc' in file[i][0] or i == file.shape[0] - 1:  # append all docs including the last one
                if (i == file.shape[0] - 1):  # append last line
                    tokens.append(int(file[i][0]), file[i][1], file[i][2],  data_utils.strToLst(file[i][3]),
                                  data_utils.

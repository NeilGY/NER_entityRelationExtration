...
def generator(data, m,config,train=False):
    ...
    maxScoringMatrixLenList0 = []
    maxScoringMatrixLen0 = -1
    maxScoringMatrixLenList1 = []
    maxScoringMatrixLen1 = -1
    ...
    for docIdx in range(len(data_copy.data)):
        ...
        if docIdx % config.batchsize == 0 and docIdx > 0:
            ...
            maxScoringMatrixLenList0.append(maxScoringMatrixLen0)
            maxScoringMatrixLen0 = -1

            maxScoringMatrixLenList1.append(maxScoringMatrixLen1)
            maxScoringMatrixLen1 = -1
        ...
        if scoringMatrix.shape[0] > maxScoringMatrixLen0:
            maxScoringMatrixLen0 = scoringMatrix.shape[0]
        if scoringMatrix.shape[1] > maxScoringMatrixLen1:
            maxScoringMatrixLen1 = scoringMatrix.shape[1]
        ...
        docs_batch_scoringMatrix.append(scoringMatrix.tolist())
        # print (scoringMatrix)
    ...
    for bIdx in range(len(batches_embeddingIds)):
        ...
        batch_scoringMatrix = batches_scoringMatrix[bIdx]

        batch_entity_tags = batches_entity_tags[bIdx]
        batch_entity_tags_ids = batches_entity_tags_ids[bIdx]
        ...
        for dIdx in range(len(batch_embeddingIds)):
            ...
            scoringMatrix_doc = batch_scoringMatrix[dIdx]

            ner_doc=batch_entity_tags[dIdx]
            ner_doc_ids=batch_entity_tags_ids[dIdx]
            token_doc = batch_tokens[dIdx]

            token_id_doc = batch_tokenIds[dIdx]

            lenEmbeddingssDoc.append(len(embeddingId_doc))
            tokensLen=[len(token) for token in charIds_doc]#每个句子中单词长度的列表
            ...
            for tokenIdx in range(len(tokensLen)):
                ...
                if tokenLen<maxWordLenList[bIdx]:
                    ...
                    for i in np.arange(maxWordLenList[bIdx]-tokenLen):
                        #print (charIds_doc)
                        charIds_doc[tokenIdx].append(0)#填充字母


            for i in np.arange(len(scoringMatrix_doc)):
                if len(scoringMatrix_doc[i]) < maxScoringMatrixLenList1[bIdx]:  # 填充列
                    for j in np.arange(maxScoringMatrixLenList1[bIdx] - len(scoringMatrix_doc[i])):
                        scoringMatrix_doc[i].append(0)

            if len(scoringMatrix_doc) < maxScoringMatrixLenList0[bIdx]:
                for i in np.arange(maxScoringMatrixLenList0[bIdx] - len(scoringMatrix_doc)):
                    scoringMatrix_doc.append([0]*maxScoringMatrixLenList1[bIdx]) #填充行

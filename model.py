...
class model:
    ...
    def train(self, train_data, operations, iter):
        ...
        for x_train in data_utils.generator(train_data, operations.m_op, self.config, train=True):
    ...
    def evaluate(self, eval_data, operations, set):
        ...
        evaluator = self.getEvaluator()
        for x_dev in data_utils.generator(eval_data, operations.m_op, self.config, train=False):
    ...
    def computeLoss(self, input_rnn, dropout_embedding_keep, dropout_lstm_keep, dropout_lstm_output_keep,
        ...
        with tf.variable_scope("loss_computation", reuse=reuse):
            ...
            for i in range(self.config.num_lstm_layers):
                ...
                #scope='BiLSTM' + str(i) 解决每层LSTM输入维度不一致问题
    ...
    def run(self):
        ...
        embedding_ids = tf.placeholder(tf.int32, [None, None])  # [ batch_size,max_length of sentence ]
